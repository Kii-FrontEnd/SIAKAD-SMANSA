...
class PengaturanController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        if ($request->hasFile('logo')) {
            if ($pengaturan->logo) {
                Storage::disk('public')->delete(str_replace('storage/', '', $pengaturan->logo));
            }

            $slug = Str::slug($pengaturan->name);
            $fileName = $slug . '_logo.' . $request->file('logo')->getClientOriginalExtension();

            // Simpan ke storage/app/public/logos
            $request->file('logo')->storeAs('logos', $fileName, 'public');

            // Simpan path publik ke database
            $pengaturan->logo = 'storage/logos/' . $fileName;
        }
